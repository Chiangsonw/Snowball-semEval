...
regex_test = re.compile('<e[1-2]>[^<]+</e[1-2]>',re.U)   #     用于匹配 <e1>xx</e1> 和<e2>xx</e2>
...
class Sentence:
    ...
    def __init__(self, _sentence, e1_type, e2_type, max_tokens, min_tokens, window_size):
        ...
        #for m in re.finditer(regex_linked, self.sentence):
        for m in re.finditer(regex_test, self.sentence):   #修改了正则表达式
            matches.append(m)
        ...
        if len(matches) >= 2:
            for x in range(0, len(matches) - 1):
                ...
                if not number_bet_tokens > max_tokens and not number_bet_tokens < min_tokens:
                    ...
                    arg1match = re.match("<e[1-2]>", ent1)
                    arg2match = re.match("<e[1-2]>", ent2)
                    ent1 = re.sub("</?e[1-2]>", "", ent1, count=2, flags=0)
                    ent2 = re.sub("</?e[1-2]>", "", ent2, count=2, flags=0)
                    arg1type = arg1match.group()[1:-1]
                    arg2type = arg2match.group()[1:-1]
                    ...
                    if e1_type is not None and e2_type is not None:
                        ...
                        if arg1type == e1_type and arg2type == e2_type:
                           # print("generated a relation")  ## debug
...
class SentenceParser:
    ...
    def __init__(self, _sentence, e1_type, e2_type):
        ...
        for m in re.finditer(regex_test, self.sentence): # chaing updated
        ...
        for e1 in self.entities:
            for e2 in self.entities:
                if e1 == e2:
                    continue
                arg1match = re.match("<(e[1-2])>", e1)
                arg2match = re.match("<(e[1-2])>", e2)
