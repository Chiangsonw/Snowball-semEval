...
class Snowball(object):
    ...
    def init_bootstrapp(self, tuples):
        ...
        for t in tmp:
            ...
            f_output.write("sentence: "+str(t.sentence)+'\n')                                                 #utf-8修改
            # write patterns that extracted this tuple
            patterns = list()               #chaing updated
            for pattern in self.candidate_tuples[t]:  #chaing updated
                patterns.append(pattern[0])   #chaing updated
            for p in patterns:
                p.merge_tuple_patterns()
    ...
    def evaluate(self):
        ...
        regex = re.compile('Cause-Effect')
        regex_sen = re.compile('\d*')
        regex_num = re.compile('\d+')
        regex_instance=re.compile('sentence')
        count_1=0
        count_2=0
        count_3=0
        train_set= set()
        ...
        with open("train_result.txt","r") as f:
            for line in f:
                ...
                if result is not None:
                    no = re.search(regex_sen,line)
                    train_set.add(no.group())
                    count_1 = count_1 + 1
        ...
        with open("relationships.txt","r") as f:
            for line in f:
                result = re.search(regex_instance, line)
                if result is not None:
                    num = re.search(regex_num,line)
                    target_set.add(num.group())
                    count_2 = count_2 + 1
        1
        f.close()
        ...
        for t in target_set:
            if t in train_set:
                count_3=count_3+1
        precision = count_3/count_2
        recall = count_2/count_1
        F_measure = precision*recall*2/(recall+precision)
...
def main():
    if len(sys.argv) != 7:
        print("\nSnowball.py parameters.cfg sentences_file seeds_file_positive seeds_file_negative similarity_threshold" \
